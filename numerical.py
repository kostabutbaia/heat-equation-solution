...
def get_solution_at_n(n: int, u_at_prev_n: list[float], source_func, u_0, u_L, k) -> list[float]:
...
def get_num_solution(source_func, u_0, u_L, g, k) -> list[list[float]]:
    u_init = [g(x) for x in x_range]
    sol = [0]*len(t_range)
    sol[0] = u_init
    for i in range(1, len(t_range)):
        sol[i] = get_solution_at_n(i, sol[i-1], source_func, u_0, u_L, k)
    return sol

def main():
    res = get_num_solution(source, u_0, u_L, g, k)
